...
if ! command -v codex-cli >/dev/null 2>&1; then
  echo "No detecté codex-cli. Instalalo y re-ejecutá ./setup.sh"; exit 1
fi
mkdir -p concept .codexkit .tmp vendor scripts
touch .codexkit/codex-check.sh .codexkit/codex-kick.sh .codexkit/codex-ops.sh \
      .codexkit/codex-gen.sh .codexkit/codex-prompt.sh .codexkit/codex-guard.sh \
